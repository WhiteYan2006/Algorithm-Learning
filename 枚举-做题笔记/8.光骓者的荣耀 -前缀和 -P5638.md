...
int main() {
  ...
  for (auto &a : ai) cin >> a;
  vector<long long> prefixPathLength(n);
  partial_sum(ai.begin(), ai.end(), prefixPathLength.begin() + 1);
  long long max_skipLength = 0;
  for (int start_index = 0; start_index < n - k; start_index++) {
    max_skipLength = max(max_skipLength, prefixPathLength[start_index + k] - prefixPathLength[start_index]);
  }
  cout << prefixPathLength[n - 1] - max_skipLength << endl;
}
```